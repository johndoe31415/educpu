...
| 10    | 4: Execute load from memory  | opcode is LDMEM, ADDMEM, ADDCMEM, SUBMEM, SUBBMEM               | Let `ADDR` be the operand of the instruction                                                                                                                                                                                                                                                                                       |
| 11    | 4: Execute load from memory  | opcode is LDMEM, ADDMEM, ADDCMEM, SUBMEM, SUBBMEM               | Issue an `MCLK` to read a byte from memory at the operand location.                                                                                                                                                                                                                                                                |
| 12    | 4: Execute load from memory  | opcode is LDMEM, ADDMEM, ADDCMEM, SUBMEM, SUBBMEM               | Replace the least significant byte of `INSN` with the read value at `DATA` and prepare to store that in the next clock cycle by asserting `WE_INSN`.                                                                                                                                                                               |
| 13    | 4: Execute load from memory  | opcode is LDMEM, ADDMEM, ADDCMEM, SUBMEM, SUBBMEM               | Issue an `RCLK` to save the value in the `INSN` register.                                                                                                                                                                                                                                                                          |
...
| 16    | 6: Execute ALU               | opcode is ADDMEM, ADDCMEM, SUBMEM, SUBBMEM, COM                 | Hand the requested operation to the ALU. Take the carry/borrow bit out of `SR` and input it into the ALU. Let ALU OP1 = `ACC` and OP2 = low operand byte of `INSN` (the value read from memory). Connect the output of the ALU to `ACC_next` and the status bits output of the ALU to `SR_next`. Assert both `WE_SR` and `WE_ACC`. |
| 17    | 6: Execute ALU               | opcode is ADDMEM, ADDCMEM, SUBMEM, SUBBMEM, COM                 | Issue a positive edge on `RCLK` to update both `ACC` and `SR`.                                                                                                                                                                                                                                                                     |
